...
async function resolveClickTarget({
  ...
  if (preferredPool.length) {
    ...
    if (assistantResult && assistantResult.status && assistantResult.status !== 'assistant') {
      console.log(`[nerovaagent] assistant fallback status=${assistantResult.status}`);
    }

    const normalizedHints = Array.isArray(hints.text_contains)
      ? hints.text_contains.map(normalizeText).filter(Boolean)
      : [];
    if (typeof hints.text_partial === 'string') {
      const partial = normalizeText(hints.text_partial);
      if (partial && !normalizedHints.includes(partial)) normalizedHints.push(partial);
    }
    const exactHintSet = new Set(exactHints);
    const desiredRoles = new Set();
    if (decision?.target?.role) desiredRoles.add(decision.target.role.toLowerCase());
    if (Array.isArray(hints.roles)) {
      for (const role of hints.roles) {
        if (role) desiredRoles.add(role.toLowerCase());
      }
    }
    const reasonText = String(decision?.reason || decision?.summary || '').toLowerCase();
    if (!desiredRoles.size) {
      if (reasonText.includes('search') || reasonText.includes('textbox') || reasonText.includes('input')) {
        desiredRoles.add('textbox');
      }
      if (reasonText.includes('button')) desiredRoles.add('button');
      if (reasonText.includes('link')) desiredRoles.add('link');
    }
    const reasonTerms = (() => {
      const terms = [];
      if (reasonText) {
        for (const token of reasonText.split(/[^a-z0-9]+/)) {
          const trimmed = token.trim();
          if (trimmed.length >= 3) terms.push(trimmed);
        }
      }
      const quoted = [];
      if (decision?.reason) {
        const regex = /'([^']+)'/g;
        let match;
        while ((match = regex.exec(decision.reason))) {
          const text = normalizeText(match[1]);
          if (text) quoted.push(text);
        }
      }
      for (const entry of quoted) {
        if (!terms.includes(entry)) terms.push(entry);
        if (!normalizedHints.includes(entry)) normalizedHints.push(entry);
      }
      return terms;
    })();
    ...
    const scoreCandidate = (element) => {
      ...
      const area = (() => {
        if (!Array.isArray(element.rect) || element.rect.length < 4) return 0;
        const w = Math.max(0, element.rect[2] || 0);
        const h = Math.max(0, element.rect[3] || 0);
        return w * h;
      })();

      if (desiredRoles.size) {
        if (desiredRoles.has(role)) score += 6;
        else if (role === 'generic') score -= 1;
        else score -= 3;
      }

      if (exactHintSet.has(name)) score += 12;
      if (!exactHintSet.size) {
        for (const term of normalizedHints) {
          if (term && name.includes(term)) { score += 6; break; }
        }
      }
      if (!exactHintSet.size) {
        for (const term of reasonTerms) {
          if (term && name.includes(term)) { score += 3; break; }
        }
      }

      if (normalizedHints.length && !normalizedHints.some((term) => name.includes(term))) {
        score -= 2;
      }

      if (element.hit_state === 'hittable') score += 3;
      else if (element.hit_state === 'occluded') score -= 3;

      if (area > 400000) score -= 6;
      else if (area > 250000) score -= 3;
      else if (area > 120000) score -= 1;
      else if (area > 10000) score += 1;

      if (center && Array.isArray(element.center) && element.center.length === 2) {
        const dist = Math.hypot(element.center[0] - center[0], element.center[1] - center[1]);
        score -= Math.min(6, dist / 80);
      }

      if (name.length > 160) score -= 2;
      return score;
    };
    ...
    if (scored.length) {
      console.log('[nerovaagent] scored candidates:', scored.slice(0, 5).map((item) => ({
        name: item.element?.name,
        role: item.element?.role,
        score: Number.isFinite(item.score) ? Number(item.score.toFixed(2)) : item.score,
        rect: item.element?.rect || null
      })));
    }

    const top = scored[0];
    if (top && Number.isFinite(top.score) && top.score > -Infinity) {
      const element = top.element;
      const centerPoint = Array.isArray(element?.center) && element.center.length === 2
        ? element.center
        : Array.isArray(element?.rect) && element.rect.length === 4
          ? [element.rect[0] + (element.rect[2] || 0) / 2, element.rect[1] + (element.rect[3] || 0) / 2]
          : [0, 0];
      console.log('[nerovaagent] scored pick chosen:', { name: element?.name, role: element?.role, center: centerPoint, score: top.score });
      return {
        status: 'ok',
        source: 'scored',
        element,
