...
app.get('/sandbox', (req, res) => {
  res.sendFile(path.join(process.cwd(), 'public', 'sandbox.html'));
});
app.get('/noplanner', (req, res) => {
  res.sendFile(path.join(process.cwd(), 'public', 'noplanner.html'));
});
app.get('/ui', (req, res) => {
...
app.post('/runtime/playwright/launch', async (req, res) => {
  ...
  try {
    await ensureBrowser();
    const viewport = page ? await page.viewportSize().catch(() => null) : null;
...
app.get('/webrtc', (req, res) => {
  res.sendFile(path.join(process.cwd(), 'public', 'webrtc.html'));
});
...
app.get('/webrtc2', async (req, res) => {
  ...
  res.sendFile(path.join(process.cwd(), 'public', 'webrtc2.html'));
});
...
async function ensureBrowser() {
  ...
  try {
    ...
    if (/ProcessSingleton/i.test(msg) || /SingletonLock/i.test(msg)) {
      try { await fs.rm(path.join(process.cwd(), 'user-data', 'SingletonLock'), { force: true }); } catch {}
      try {
        browser = await chromium.launchPersistentContext('./user-data', {
          headless: !headful,
          viewport: { width: vw, height: vh },
...
app.post('/recipes/journal/append', async (req, res) => {
  try {
    ...
    const file = path.join(process.cwd(), 'data', 'tmp', `${plan_id}.json`);
    const buf = await fs.readFile(file, 'utf8');
...
app.post('/recipes/journal/finalize', async (req, res) => {
  try {
    const { plan_id } = req.body || {};
    if (!plan_id) { res.status(400).json({ ok:false, error:'Missing plan_id' }); return; }
    const tmpFile = path.join(process.cwd(), 'data', 'tmp', `${plan_id}.json`);
